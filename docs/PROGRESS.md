...
## Current
Phase 2 — IN PROGRESS (Tasks 2.1–2.4 complete; Task 2.5 frontend code written but NOT committed/tested). Phase 1 deploy (Task 1.6) still blocked on server `.env` creation with user-chosen dashboard password.

**Task 2.5 status:** `lib/sse.ts`, `api/types.ts`, `api/fixtures.ts`, `components/feed/{EventRow,Feed}.tsx`, `components/cards/AgentCard.tsx`, `pages/{Agent,Sessions,SessionDetail}.tsx`, `pages/MissionControl.tsx` updated, `routes.tsx` + `Nav.tsx` updated. Still needed: frontend tests (`feed.test.tsx`, `agentcard.test.tsx`, `chat.test.tsx`), `pnpm exec vitest run`, `pnpm build`, commit.
